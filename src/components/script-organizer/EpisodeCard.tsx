...
export function EpisodeCard({ summary }: EpisodeCardProps) {
  ...
  return (
    <div className="bg-[#12122a] border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition-colors">
      {/* 헤더 */}
      <div className="px-5 py-3 border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="text-2xl font-bold text-white/90">{summary.episode}</span>
          <div>
            <p className="text-xs text-white/40">EPISODE</p>
            <p className="text-xs text-white/30 truncate max-w-[200px]" title={summary.fileName}>
              {summary.fileName}
            </p>
          </div>
        </div>
      ...
      <div className="px-5 py-3">
        <p className="text-sm text-white/70 leading-relaxed">{summary.plotSummary}</p>
      </div>
      ...
      {summary.emotionalArc && (
        <div className="px-5 py-2 border-t border-white/5">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-white/50">{summary.emotionalArc}</p>
          </div>
        </div>
      )}
