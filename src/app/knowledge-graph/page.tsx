    ...
    function tick() {
      ...
      for (const edge of edges) {
        ...
        if (edge.type === 'owns') {
          ...
          const brightness = Math.min(edge.weight * 1.5, 1);
          ctx.strokeStyle = `rgba(255, 180, 50, ${brightness * 0.6})`;
      ...
      for (const node of nodes) {
        ...
        if (node.type === 'assistant') {
          ...
          ctx.fillStyle = node.docType === 'reference' ? '#f59e0b88' : `${color}66`;
        }
        ctx.fill();

        ctx.strokeStyle = `${color}cc`;
        ctx.lineWidth = node.type === 'assistant' ? 2.5 : 1.5;
        ctx.stroke();
        ...
        if (node.type === 'assistant') {
          ...
          ctx.fillText(node.docType === 'reference' ? '📄' : '📝', node.x!, node.y! - 1);
        }
    ...
    <div className="min-h-screen bg-[#0a0a1a] flex flex-col">
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-[#0a0a1a]/90 backdrop-blur-md z-10">
        ...
        <div className="flex items-center gap-6 text-sm text-white/60">
          <span>문서 <b className="text-indigo-400">{graphData?.stats.totalDocuments || 0}</b></span>
          <span>보조작가 <b className="text-purple-400">{graphData?.stats.totalAssistants || 0}</b></span>
          <span>연결 <b className="text-amber-400">{graphData?.stats.totalEdges || 0}</b></span>
        </div>
      </header>
      ...
      <div className="flex-1 relative">
        ...
        {/* 컨트롤 패널 */}
        <div className="absolute top-4 left-4 bg-[#12122a]/90 backdrop-blur-md border border-white/10 rounded-xl p-4 w-64 space-y-4">
          <div>
            ...
            <div className="flex justify-between text-xs text-white/40 mt-1">
              <span>느슨함 0.1</span>
              <span className="text-indigo-400 font-medium">{threshold.toFixed(2)}</span>
              <span>엄격함 0.9</span>
            </div>
          </div>
          ...
          <div className="border-t border-white/5 pt-3">
            ...
            <div className="space-y-1.5 text-xs">
              <div className="flex items-center gap-2">
                <span className="w-4 h-4 rounded-full bg-indigo-500/60 border border-indigo-400/50 inline-block" />
                <span className="text-white/60">보조작가</span>
              </div>
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-indigo-400/40 border border-indigo-300/40 inline-block" />
                <span className="text-white/60">대본 문서</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-amber-400/40 border border-amber-300/40 inline-block" />
                <span className="text-white/60">참고 자료</span>
              </div>
              <div className="flex items-center gap-2 mt-2">
                <span className="w-6 border-t border-dashed border-blue-400/40 inline-block" />
                <span className="text-white/60">소유 관계</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-6 border-t-2 border-amber-400/60 inline-block" />
        ...
        {hoveredNode && (
          <div className="absolute bottom-4 left-4 bg-[#12122a]/95 backdrop-blur-md border border-white/10 rounded-xl p-4 max-w-xs">
            ...
            <p className="text-white/40 text-xs mt-1">
              {hoveredNode.type === 'assistant' && `보조작가 · ${hoveredNode.specialty}`}
              {hoveredNode.type === 'knowledge_base' && '공유 지식 베이스'}
              {hoveredNode.type === 'document' && `${hoveredNode.docType === 'reference' ? '참고자료' : '대본'} · ${hoveredNode.chunkCount || 0}개 청크`}
            </p>
          </div>
        )}
