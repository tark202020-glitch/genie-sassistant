...
export async function POST(req: Request) {
  try {
    ...
    // 1. Supabase에 보조작가 프로필 생성 (먼저 ID 확보)
    ...
    const gcsFolder = `assistants/${assistantId}`;

    console.log(`[Assistants] 보조작가 생성: ${name} (${assistantId})`);
    ...
    // 3. Supabase에 GCS 폴더 정보 업데이트
    // 기존 story-ai-helper 데이터 스토어를 공유 사용 (별도 데이터 스토어 생성 없음)
    // Import는 기존 데이터 스토어에 하되, GCS 하위 폴더로 분리 관리
    ...
    return NextResponse.json({
      ...
      message: `보조작가 "${name}" 생성 완료! 전용 폴더가 준비되었습니다.`,
    });
  } catch (err: any) {
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: '삭제할 보조작가 ID가 필요합니다.' }, { status: 400 });
    }
    ...
    console.log(`[Assistants] 보조작가 삭제 시작: ${assistant.name} (${id})`);
    ...
    return NextResponse.json({
      success: true,
      message: `보조작가 "${assistant.name}" 삭제 완료.`,
    });
  } catch (err: any) {
    console.error('[Assistants Exception]', err);
    return NextResponse.json({ error: err.message }, { status: 500 });
