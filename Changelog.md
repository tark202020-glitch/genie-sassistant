...
- **Summary**: 로그아웃 리다이렉트, 로고 이미지 적용, 브랜드명 변경, 대본 정리 전면 리뉴얼, 저장 기능
- **Detail**:
  - ✅ 로그아웃/미인증 시 `/login` → `/landing` 리다이렉트
  - ✅ PNG 로고 다크/라이트 자동 전환 (PildongLogoImg 컴포넌트)
  - ✅ 브랜드명 "필동" → "지작" 전체 통일
  - ✅ 캐릭터 탭: Canvas/Chord 관계도 → 텍스트 프로필 카드 (나이/직업/목적/성격/관계)
  - ✅ 배경 탭: Canvas 다이어그램 → 유형별 텍스트 목록
  - ✅ 3개 탭 수동 저장하기 버튼 + 캐시 우선 로드 (자동 분석 제거)
  - ✅ script_analyses RLS 정책 누락 해결 (저장 실패 원인)
  - ✅ Apple touch icon 설정 (iOS 홈 화면 아이콘)
  - ✅ 캐릭터 관계도 Canvas 렌더링 버그 수정 (ResizeObserver 의존성)
